...
mod tests {
    ...
    fn upgradable_enum_required() {
        ...
        let serialized_bytes = OuterOuterStruct {
            outer_struct: Some(OuterStructRequiredEnum {
                inner_enum: dummy_inner_enum,
            }),
            other_field: 0x1bad1dea,
        }
        .encode();
    ...
    fn bolt_tlv_bogus_n1_stream() {
        ...
        do_test!(
            concat!("02", "08", "0000000000000226", "01", "01", "2a"),
            InvalidValue
        );
        do_test!(
            concat!(
        ...
        do_test!(
            concat!("ffffffffffffffffff", "00", "01", "00"),
            InvalidValue
        );
    }
    ...
    fn do_simple_test_tlv_write() -> Result<(), io::Error> {
        ...
        assert_eq!(
            stream.0,
            <Vec<u8>>::from_hex("0e00080000000000000001fd00ff00").unwrap()
        );
    ...
    fn test_impl_writeable_enum() {
        ...
        assert_eq!(
            TuplesOnly::read(&mut Cursor::new(&a)).unwrap(),
            Some(TuplesOnly::A())
        );
        let b42 = TuplesOnly::B(42).encode();
        assert_eq!(
            TuplesOnly::read(&mut Cursor::new(&b42)).unwrap(),
            Some(TuplesOnly::B(42))
        );
